...
{
    [Serializable]
    [NodeAttributes("Function Output", "Functions",
    ...
    {
        public FunctionOutput() : base()
        {
            CommonInit();
        }

        public FunctionOutput(int uniqueId, float x, float y, float width, float height) : base(uniqueId, x, y, width,
            height)
        {
            CommonInit();
        }

        [SerializeField] private bool m_previewNode = false;

        [SerializeField] private string m_outputName = "Output";

        [SerializeField] private int m_orderIndex = -1;

        [SerializeField] private AmplifyShaderFunction m_function;
        ...
        public override void OnConnectedOutputNodeChanges(int outputPortId, int otherNodeId, int otherPortId,
            string name, WirePortDataType type)
        {
        ...
        [SerializeField] private string m_currentTitle = string.Empty;
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            SetAdditonalTitleTextOnCallback(m_currentTitle,
                (instance, newSubTitle) => instance.AdditonalTitleContent.text = newSubTitle);
            ...
            if (ContainerGraph.LodLevel <= ParentGraph.NodeLOD.LOD3)
            {
                ...
                if (m_isEditing || m_startEditing)
                {
                    ...
                    m_outputName = EditorGUITextField(m_titleClickArea, string.Empty, m_outputName,
