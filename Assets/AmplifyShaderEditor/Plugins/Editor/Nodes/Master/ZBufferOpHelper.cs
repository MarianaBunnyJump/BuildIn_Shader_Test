...
namespace AmplifyShaderEditor
{
    ...
    class ZBufferOpHelper
    {
        ...
        [SerializeField] private InlineProperty m_zTestMode = new InlineProperty();

        [SerializeField] private InlineProperty m_zWriteMode = new InlineProperty();
        [SerializeField] private InlineProperty m_offsetFactor = new InlineProperty();

        [SerializeField] private InlineProperty m_offsetUnits = new InlineProperty();

        [SerializeField] private bool m_offsetEnabled;

        [SerializeField] private bool m_extraDepthPass;

        [SerializeField] private int m_extrazTestMode = 0;
        ...
        public string CreateDepthInfo(bool outlineZWrite, bool outlineZTest)
        {
            ...
            {
                MasterNode.AddRenderState(ref result, "ZWrite",
                    m_zWriteMode.GetValueOrProperty(ZWriteModeValues[m_zWriteMode.IntValue]));
            }
            ...
            {
                MasterNode.AddRenderState(ref result, "ZTest",
                    m_zTestMode.GetValueOrProperty(ZTestModeValues[m_zTestMode.IntValue]));
            }
            ...
            if (m_offsetEnabled)
            {
                MasterNode.AddRenderState(ref result, "Offset ",
                    m_offsetFactor.GetValueOrProperty() + " , " + m_offsetUnits.GetValueOrProperty());
            }
        ...
        public void Draw(UndoParentNode owner, GUIStyle toolbarstyle, bool customBlendAvailable)
        {
            ...
            m_parentSurface.ContainerGraph.ParentWindow.InnerWindowVariables.ExpandedDepth = owner.GUILayoutToggle(
                m_parentSurface.ContainerGraph.ParentWindow.InnerWindowVariables.ExpandedDepth, DepthParametersStr,
                UIUtils.MenuItemToggleStyle);
            if (EditorGUI.EndChangeCheck())
            {
                EditorPrefs.SetBool("ExpandedDepth",
                    m_parentSurface.ContainerGraph.ParentWindow.InnerWindowVariables.ExpandedDepth);
            }
            ...
            if (m_parentSurface.ContainerGraph.ParentWindow.InnerWindowVariables.ExpandedDepth)
            {
                ...
                if (!customBlendAvailable)
                    EditorGUILayout.HelpBox("Depth Writing is only available for Opaque or Custom blend modes",
                        MessageType.Warning);
        ...
        public bool IsActive
        {
            get
            {
                return m_zTestMode.IntValue != 0 || m_zWriteMode.IntValue != 0 || m_offsetEnabled ||
                       m_zTestMode.Active || m_zWriteMode.Active;
            }
        }
