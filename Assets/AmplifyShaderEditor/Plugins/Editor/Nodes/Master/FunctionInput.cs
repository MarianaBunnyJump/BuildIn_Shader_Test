...
{
    [Serializable]
    [NodeAttributes("Function Input", "Functions", "Function Input adds an input port to the shader function",
    ...
    public sealed class FunctionInput : ParentNode
    {
        ...
        private readonly string[] m_inputValueTypes =
        {
            "Int",
            "Float",
            "Vector2",
        ...
        [SerializeField] private int m_selectedInputTypeInt = 1;

        private WirePortDataType m_selectedInputType = WirePortDataType.FLOAT;

        [SerializeField] private FunctionNode m_functionNode;

        [SerializeField] private string m_inputName = "Input";

        [SerializeField] private bool m_autoCast = false;
        ...
        //Title editing 
        [SerializeField] private string m_uniqueName;
        ...
        public override void SetPreviewInputs()
        {
            ...
            else
            {
                ...
                if (input != null && (!InputPorts[0].IsConnected || input.IsConnected))
                {
                    ...
                    PreviewMaterial.SetTexture(InputPorts[0].CachedPropertyId,
                        input.InputPreviewTexture(Fnode.ContainerGraph));
                }
                else
                {
                    m_ignoreConnection = false;
        ...
        public override void OnInputPortConnected(int portId, int otherNodeId, int otherPortId,
            bool activateNode = true)
        {
        ...
        public override void OnConnectedOutputNodeChanges(int portId, int otherNodeId, int otherPortId, string name,
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            {
                if (!m_isEditing && ((!ContainerGraph.ParentWindow.MouseInteracted &&
                                      drawInfo.CurrentEventType == EventType.MouseDown &&
                                      m_titleClickArea.Contains(drawInfo.MousePosition))))
                {
                    ...
                    m_clickTime = EditorApplication.timeSinceStartup;
                         ...
                         ((drawInfo.CurrentEventType == EventType.MouseDown &&
                           !m_titleClickArea.Contains(drawInfo.MousePosition)) || !EditorGUIUtility.editingTextField))
                {
                    m_stopEditing = true;
                }
                ...
                if (m_isEditing || m_startEditing)
                {
                    ...
                    m_inputName = EditorGUITextField(m_titleClickArea, string.Empty, m_inputName,
        ...
        public override void OnNodeRepaint(DrawInfo drawInfo)
        {
            ...
            if (m_showTitleWhenNotEditing && !m_isEditing && !m_startEditing &&
                ContainerGraph.LodLevel <= ParentGraph.NodeLOD.LOD3)
            {
        ...
        {
            if (currentMousePos2D.y - m_globalPosition.y >
                (Constants.NODE_HEADER_HEIGHT + Constants.NODE_HEADER_EXTRA_HEIGHT) *
                ContainerGraph.ParentWindow.CameraDrawInfo.InvertedZoom)
            {
                ContainerGraph.ParentWindow.ParametersWindow.IsMaximized =
                    !ContainerGraph.ParentWindow.ParametersWindow.IsMaximized;
            }
        }
        ...
        void UpdatePorts()
        {
            ...
            m_outputPorts[0]
                .ChangeTypeWithRestrictions(m_selectedInputType, PortCreateRestriction(m_selectedInputType));
            m_sizeIsDirty = true;
        }
        ...
        public int PortCreateRestriction(WirePortDataType dataType)
        {
            ...
            switch (dataType)
            {
                ...
                {
                    types = new WirePortDataType[]
                    {
                        WirePortDataType.FLOAT, WirePortDataType.FLOAT2, WirePortDataType.FLOAT3,
                        WirePortDataType.FLOAT4, WirePortDataType.COLOR, WirePortDataType.INT, WirePortDataType.OBJECT
                    };
                }
                    break;
                ...
                {
                    types = new WirePortDataType[]
                        { WirePortDataType.FLOAT3x3, WirePortDataType.FLOAT4x4, WirePortDataType.OBJECT };
                }
                    break;
                ...
                case WirePortDataType.SAMPLER2DARRAY:
                {
                    types = new WirePortDataType[]
                    {
                        WirePortDataType.SAMPLER1D, WirePortDataType.SAMPLER2D, WirePortDataType.SAMPLER3D,
                        WirePortDataType.SAMPLERCUBE, WirePortDataType.SAMPLER2DARRAY, WirePortDataType.OBJECT
                    };
                }
                    break;
