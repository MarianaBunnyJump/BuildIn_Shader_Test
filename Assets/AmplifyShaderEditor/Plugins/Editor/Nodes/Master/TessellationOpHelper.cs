...
namespace AmplifyShaderEditor
{
    ...
    public sealed class TessellationOpHelper
    {
        ...
        private readonly string[] TesselationTypeLabels =
            { "Distance-based", "Fixed", "Edge Length", "Edge Length Cull" };
        ...
        private const string TessFunctionOpen =
        ...
        private const string DistBasedTessFunctionBody =
        ...
        // Edge Length
        private GUIContent EdgeLengthContent = new GUIContent("Edge Length",
        ...
        private const string EdgeLengthTessFunctionBody =
            "\t\t\treturn UnityEdgeLengthBasedTess (v0.vertex, v1.vertex, v2.vertex, _EdgeLength);\n";

        private const string EdgeLengthTessCullFunctionBody =
        ...
        // Phong
        private GUIContent PhongEnableContent = new GUIContent("Phong",
        ...
        private const string PhongStrengthProperty =
        ...
        [SerializeField] private int m_tessType = 2;

        [SerializeField] private float m_tessMinDistance = 10f;

        [SerializeField] private float m_tessMaxDistance = 25f;

        [SerializeField] private float m_tessFactor = 15f;

        [SerializeField] private float m_phongStrength = 0.5f;

        [SerializeField] private bool m_phongEnabled = false;

        [SerializeField] private string[] m_customData = { string.Empty, string.Empty, string.Empty };

        [SerializeField] private bool m_hasCustomFunction = false;

        [SerializeField] private string m_customFunction = String.Empty;

        [SerializeField] private string m_additionalData = string.Empty;

        [SerializeField] private StandardSurfaceOutputNode m_parentSurface;
        ...
        public void Draw(UndoParentNode owner, GUIStyle toolbarstyle, Material mat, bool connectedInput)
        {
            ...
            m_parentSurface.ContainerGraph.ParentWindow.InnerWindowVariables.ExpandedTesselation = GUILayout.Toggle(
                m_parentSurface.ContainerGraph.ParentWindow.InnerWindowVariables.ExpandedTesselation, " Tessellation",
                UIUtils.MenuItemToggleStyle, GUILayout.ExpandWidth(true));
            if (EditorGUI.EndChangeCheck())
            {
                EditorPrefs.SetBool("ExpandedTesselation",
                    m_parentSurface.ContainerGraph.ParentWindow.InnerWindowVariables.ExpandedTesselation);
            }

            EditorGUI.BeginChangeCheck();
            m_enabled = owner.EditorGUILayoutToggle(string.Empty, m_enabled, UIUtils.MenuItemEnableStyle,
                GUILayout.Width(16));
            if (EditorGUI.EndChangeCheck())
            {
                if (m_enabled)
            ...
            if (m_parentSurface.ContainerGraph.ParentWindow.InnerWindowVariables.ExpandedTesselation)
            {
                ...
                GUI.color = new Color(cachedColor.r, cachedColor.g, cachedColor.b,
                    (EditorGUIUtility.isProSkin ? 0.5f : 0.25f));
                ...
                m_tessType = owner.EditorGUILayoutIntPopup(TesselationTypeStr, m_tessType, TesselationTypeLabels,
                    TesselationTypeValues);
                ...
                switch (m_tessType)
                {
                    ...
                    case 3:
                    {
                        ...
                        EditorGUI.BeginChangeCheck();
                        m_tessMaxDistance =
                            owner.EditorGUILayoutFloatField(EdgeLengthTessMaxDisplacementContent, m_tessMaxDistance);
                        if (EditorGUI.EndChangeCheck() && mat != null)
                        {
                            if (mat.HasProperty(TessMinUniformName))
            ...
            switch (m_tessType)
            {
                case 0:
                {
                    if (!m_hasCustomFunction)
                    {
                        //Tess
                        uniforms += "\t\tuniform " +
                                    UIUtils.PrecisionWirePortToCgType(PrecisionType.Float, WirePortDataType.FLOAT) +
                                    " " + TessUniformName + ";\n";
        ...
        public void AddToDataCollector(ref MasterNodeDataCollector dataCollector, int reorder)
        {
            ...
            switch (m_tessType)
            {
                case 0:
                {
                    ...
                    if (!m_hasCustomFunction)
                    {
                        ...
                        dataCollector.AddToUniforms(-1,
                            "uniform " +
                            UIUtils.PrecisionWirePortToCgType(PrecisionType.Float, WirePortDataType.FLOAT) + " " +
                            TessUniformName + ";");

                        //Min
                        dataCollector.AddToProperties(-1, string.Format(TessMinProperty, m_tessMinDistance),
                            orderIndex++);
                        dataCollector.AddToUniforms(-1,
                            "uniform " +
                            UIUtils.PrecisionWirePortToCgType(PrecisionType.Float, WirePortDataType.FLOAT) + " " +
                        ...
                        //Max
                        dataCollector.AddToProperties(-1, string.Format(TessMaxProperty, m_tessMaxDistance),
                            orderIndex++);
                        dataCollector.AddToUniforms(-1,
                            "uniform " +
                            UIUtils.PrecisionWirePortToCgType(PrecisionType.Float, WirePortDataType.FLOAT) + " " +
                ...
                case 2:
                {
                    ...
                    if (!m_hasCustomFunction)
                    {
                        dataCollector.AddToProperties(-1, string.Format(EdgeLengthTessProperty, m_tessFactor),
                            orderIndex++);
                        dataCollector.AddToUniforms(-1,
                            "uniform " +
                            UIUtils.PrecisionWirePortToCgType(PrecisionType.Float, WirePortDataType.FLOAT) + " " +
                            EdgeLengthTessUniformName + ";");
                    }
                }
                    break;
                case 3:
                {
                    ...
                    if (!m_hasCustomFunction)
                    {
                        ...
                        //Max Displacement
                        dataCollector.AddToProperties(-1,
                            string.Format(EdgeLengthTessMaxDispProperty, m_tessMaxDistance), orderIndex++);
                        dataCollector.AddToUniforms(-1,
                            "uniform " +
                            UIUtils.PrecisionWirePortToCgType(PrecisionType.Float, WirePortDataType.FLOAT) + " " +
                            EdgeLengthTessMaxDispUniformName + ";");
                    }
                }
                    break;
            }
            ...
            if (m_phongEnabled)
            {
                ...
                dataCollector.AddToUniforms(-1,
                    "uniform " + UIUtils.PrecisionWirePortToCgType(PrecisionType.Float, WirePortDataType.FLOAT) + " " +
                    PhongStrengthUniformName + ";");
            }
        }
        ...
        public StandardSurfaceOutputNode ParentSurface
        {
            get { return m_parentSurface; }
            set { m_parentSurface = value; }
        }
    }
}