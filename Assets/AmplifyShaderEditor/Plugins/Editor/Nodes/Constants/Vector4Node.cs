...
namespace AmplifyShaderEditor
{
    ...
    {
        [SerializeField] private Vector4 m_defaultValue = Vector4.zero;

        [SerializeField] private Vector4 m_materialValue = Vector4.zero;
        ...
        public Vector4Node(int uniqueId, float x, float y, float width, float height) : base(uniqueId, x, y, width,
            height)
        {
        }
        ...
        public override void SetPreviewInputs()
        {
            ...
            if (m_materialMode && m_currentParameterType != PropertyType.Constant)
                PreviewMaterial.SetVector(m_cachedPropertyId,
                    new Vector4(m_materialValue[0], m_materialValue[1], m_materialValue[2], m_materialValue[3]));
            else
                PreviewMaterial.SetVector(m_cachedPropertyId,
                    new Vector4(m_defaultValue[0], m_defaultValue[1], m_defaultValue[2], m_defaultValue[3]));
        }
        ...
        public override void ConfigureLocalVariable(ref MasterNodeDataCollector dataCollector)
        {
            ...
            dataCollector.AddLocalVariable(UniqueId,
                CreateLocalVarDec(value.x + "," + value.y + "," + value.z + "," + value.w));
        ...
        {
            return PropertyAttributes + m_propertyName + "(\"" + m_propertyInspectorName + "\", Vector) = (" +
                   m_defaultValue.x + "," + m_defaultValue.y + "," + m_defaultValue.z + "," + m_defaultValue.w + ")";
        }
        ...
        public override string GetPropertyValStr()
        {
            return (m_materialMode && m_currentParameterType != PropertyType.Constant)
                ? m_materialValue.x.ToString(Mathf.Abs(m_materialValue.x) > 1000
                      ? Constants.PropertyBigVectorFormatLabel
                      : Constants.PropertyVectorFormatLabel) + IOUtils.VECTOR_SEPARATOR +
                  m_materialValue.y.ToString(Mathf.Abs(m_materialValue.y) > 1000
                      ? Constants.PropertyBigVectorFormatLabel
                      : Constants.PropertyVectorFormatLabel) + IOUtils.VECTOR_SEPARATOR +
                  m_materialValue.z.ToString(Mathf.Abs(m_materialValue.z) > 1000
                      ? Constants.PropertyBigVectorFormatLabel
                      : Constants.PropertyVectorFormatLabel) + IOUtils.VECTOR_SEPARATOR +
                  m_materialValue.w.ToString(Mathf.Abs(m_materialValue.w) > 1000
                      ? Constants.PropertyBigVectorFormatLabel
                      : Constants.PropertyVectorFormatLabel)
                : m_defaultValue.x.ToString(Mathf.Abs(m_defaultValue.x) > 1000
                      ? Constants.PropertyBigVectorFormatLabel
                      : Constants.PropertyVectorFormatLabel) + IOUtils.VECTOR_SEPARATOR +
                  m_defaultValue.y.ToString(Mathf.Abs(m_defaultValue.y) > 1000
                      ? Constants.PropertyBigVectorFormatLabel
                      : Constants.PropertyVectorFormatLabel) + IOUtils.VECTOR_SEPARATOR +
                  m_defaultValue.z.ToString(Mathf.Abs(m_defaultValue.z) > 1000
                      ? Constants.PropertyBigVectorFormatLabel
                      : Constants.PropertyVectorFormatLabel) + IOUtils.VECTOR_SEPARATOR +
                  m_defaultValue.w.ToString(Mathf.Abs(m_defaultValue.w) > 1000
                      ? Constants.PropertyBigVectorFormatLabel
                      : Constants.PropertyVectorFormatLabel);
        }
