...
namespace AmplifyShaderEditor
{
    ...
    public sealed class ColorNode : PropertyNode
    {
        ...
        [SerializeField] private bool m_isHDR = false;
        ...
        [SerializeField] private bool m_autoGammaToLinearConversion = true;
        ...
        public ColorNode(int uniqueId, float x, float y, float width, float height) : base(uniqueId, x, y, width,
            height)
        {
        }
...
#if UNITY_2018_1_OR_NEWER
            m_defaultValue = EditorGUILayoutColorField(Constants.DefaultValueLabelContent, m_defaultValue, false, true,
                m_isHDR);
#else
			m_defaultValue =
 EditorGUILayoutColorField( Constants.DefaultValueLabelContent, m_defaultValue, false, true, m_isHDR, m_hdrConfig );
#endif
            if (m_currentParameterType == PropertyType.Constant)
            {
                m_autoGammaToLinearConversion =
...
#if UNITY_2018_1_OR_NEWER
            m_materialValue = EditorGUILayoutColorField(Constants.MaterialValueLabelContent, m_materialValue, false,
                true, m_isHDR);
#else
			m_materialValue =
 EditorGUILayoutColorField( Constants.MaterialValueLabelContent, m_materialValue, false, true, m_isHDR, m_hdrConfig );
#endif
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            if (m_isEditingFields && m_currentParameterType != PropertyType.Global)
            {
                if (m_materialMode && m_currentParameterType != PropertyType.Constant)
                {
                    EditorGUI.BeginChangeCheck();
#if UNITY_2018_1_OR_NEWER
                    m_materialValue = EditorGUIColorField(m_propertyDrawPos, m_dummyContent, m_materialValue, false,
                        true, m_isHDR);
#else
					m_materialValue =
 EditorGUIColorField( m_propertyDrawPos, m_dummyContent, m_materialValue, false, true, m_isHDR, m_hdrConfig );
#endif
                ...
                else
                {
                    EditorGUI.BeginChangeCheck();
#if UNITY_2018_1_OR_NEWER
                    m_defaultValue = EditorGUIColorField(m_propertyDrawPos, m_dummyContent, m_defaultValue, false, true,
                        m_isHDR);
#else
					m_defaultValue =
 EditorGUIColorField( m_propertyDrawPos, m_dummyContent, m_defaultValue, false, true, m_isHDR, m_hdrConfig );
#endif
                    if (EditorGUI.EndChangeCheck())
                    {
                        PreviewIsDirty = true;
        ...
        public override void ConfigureLocalVariable(ref MasterNodeDataCollector dataCollector)
        {
            ...
            dataCollector.AddLocalVariable(UniqueId,
                CreateLocalVarDec(color.r + "," + color.g + "," + color.b + "," + color.a));
            ...
            bool ignoreLocalvar)
        {
            ...
            if (m_autoGammaToLinearConversion)
            {
                ...
                string colorGamma = m_precisionString + "(" + m_defaultValue.r + "," + m_defaultValue.g + "," +
                                    m_defaultValue.b + "," + m_defaultValue.a + ")";
        ...
        void CheckIfHDR()
        {
            ...
            if (hdrBuffer && !m_isHDR)
            {
                ...
                {
                    float defaultColorLength = Mathf.Sqrt(m_defaultValue.r * m_defaultValue.r +
                                                          m_defaultValue.g * m_defaultValue.g +
                                                          m_defaultValue.b * m_defaultValue.b);
                ...
                {
                    float materialColorLength = Mathf.Sqrt(m_materialValue.r * m_materialValue.r +
                                                           m_materialValue.g * m_materialValue.g +
                                                           m_materialValue.b * m_materialValue.b);
        ...
        {
            return PropertyAttributes + m_propertyName + "(\"" + m_propertyInspectorName + "\", Color) = (" +
                   m_defaultValue.r + "," + m_defaultValue.g + "," + m_defaultValue.b + "," + m_defaultValue.a + ")";
        }
        ...
        {
            Shader.SetGlobalColor(m_propertyName, m_defaultValue);
        }

        public override void FetchGlobalValue()
        {
            m_materialValue = Shader.GetGlobalColor(m_propertyName);
        }

        public override string GetPropertyValStr()
        {
            return (m_materialMode && m_currentParameterType != PropertyType.Constant)
                ? m_materialValue.r.ToString(Constants.PropertyVectorFormatLabel) + IOUtils.VECTOR_SEPARATOR +
