...
namespace AmplifyShaderEditor
{
    ...
    public sealed class ScreenColorNode : PropertyNode
    {
        ...
        [SerializeField] private TexReferenceType m_referenceType = TexReferenceType.Object;

        [SerializeField] private int m_referenceArrayId = -1;

        [SerializeField] private int m_referenceNodeId = -1;

        [SerializeField] private GUIStyle m_referenceIconStyle = null;

        private ScreenColorNode m_referenceNode = null;

        [SerializeField] private bool m_normalize = false;

        [SerializeField] private bool m_useCustomGrab = false;

        [SerializeField] private float m_referenceWidth = -1;
        ...
        public ScreenColorNode(int uniqueId, float x, float y, float width, float height) : base(uniqueId, x, y, width,
            height)
        {
        }
        ...
        public override void DrawMainPropertyBlock()
        {
            ...
            m_referenceType = (TexReferenceType)EditorGUILayoutPopup(Constants.ReferenceTypeStr, (int)m_referenceType,
                Constants.ReferenceArrayLabels);
        ...
        public override void DrawTitle(Rect titlePos)
        {
            ...
            else if (ContainerGraph.LodLevel <= ParentGraph.NodeLOD.LOD3)
            {
                SetAdditonalTitleTextOnCallback(GrabTextureDefault,
                    (instance, newSubTitle) => instance.AdditonalTitleContent.text =
                        string.Format(Constants.SubTitleVarNameFormatStr, newSubTitle));
            ...
            bool ignoreLocalVar)
        {
            ...
            if (dataCollector.IsSRP)
            {
                ...
                {
                    dataCollector.AddLocalVariable(UniqueId, CurrentPrecisionType, WirePortDataType.FLOAT4, valueName,
                        string.Format(LWFetchOpaqueTexture, uvCoords));
                }
                else
                {
#if UNITY_2018_3_OR_NEWER
                    if (ASEPackageManagerHelper.CurrentHDVersion >= ASESRPVersions.ASE_SRP_5_13_0)
                    {
                        ...
                        dataCollector.AddLocalVariable(UniqueId, m_currentPrecisionType, WirePortDataType.FLOAT4,
                            valueName,
                            string.Format(HDSampleSceneColorHeader5, uvCoords, "0",
                    ...
                    else
                    {
                        ...
                        dataCollector.AddLocalVariable(UniqueId, m_currentPrecisionType, WirePortDataType.FLOAT4,
                            valueName, string.Format(HDSampleSceneColorHeader4, uvCoords));
                    }
#endif
                }
            }
        ...
        public string SetFetchedData(ref MasterNodeDataCollector dataCollector, bool ignoreLocalVar)
        {
            ...
            if (ignoreLocalVar)
            {
                string samplerValue = SamplerType + (isProjecting ? "proj" : "") + "( " + propertyName + ", " +
                                      GetUVCoords(ref dataCollector, ignoreLocalVar, isProjecting) + " )";
                return samplerValue;
            }
...
#else
			string samplerOp =
 SamplerType + ( isProjecting ? "proj" : "" ) + "( " + propertyName + ", " + uvValue + " )";
#endif
            dataCollector.AddLocalVariable(UniqueId,
                UIUtils.PrecisionWirePortToCgType(CurrentPrecisionType, m_outputPorts[0].DataType) + " " +
                ScreenColorStr + OutputId + " = " + samplerOp + ";");
        ...
        private string GetUVCoords(ref MasterNodeDataCollector dataCollector, bool ignoreLocalVar, bool isProjecting)
        {
            ...
            {
                result = m_inputPorts[0].GenerateShaderForOutput(ref dataCollector,
                    (isProjecting ? WirePortDataType.FLOAT4 : WirePortDataType.FLOAT2), ignoreLocalVar, true);
            }
            else
            {
                ...
                if (isProjecting)
                    result = GeneratorUtils.GenerateGrabScreenPosition(ref dataCollector, UniqueId,
                        CurrentPrecisionType, !dataCollector.UsingCustomScreenPos, customScreenPos);
                else
                    result = GeneratorUtils.GenerateGrabScreenPositionNormalized(ref dataCollector, UniqueId,
                        CurrentPrecisionType, !dataCollector.UsingCustomScreenPos, customScreenPos);
            }
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            {
                m_referenceType =
                    (TexReferenceType)Enum.Parse(typeof(TexReferenceType), GetCurrentParam(ref nodeParams));
