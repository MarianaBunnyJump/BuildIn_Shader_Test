...
namespace AmplifyShaderEditor
{
    ...
    public sealed class TexelSizeNode : ParentNode
    {
        ...
        [SerializeField] private int m_referenceSamplerId = -1;

        [SerializeField] private int m_referenceNodeId = -1;

        [SerializeField] private TexturePropertyNode m_inputReferenceNode = null;

        private TexturePropertyNode m_referenceNode = null;

        private UpperLeftWidgetHelper m_upperLeftWidget = new UpperLeftWidgetHelper();

        [SerializeField] private string m_previousTexture = string.Empty;
        ...
        protected override void CommonInit(int uniqueId)
        {
            ...
            m_inputPorts[0].CreatePortRestrictions(WirePortDataType.SAMPLER1D, WirePortDataType.SAMPLER2D,
                WirePortDataType.SAMPLER3D, WirePortDataType.SAMPLERCUBE, WirePortDataType.SAMPLER2DARRAY,
        ...
        void UpdateTitle()
        {
            ...
            {
                m_additionalContent.text = string.Format(Constants.PropertyValueLabel,
                    m_inputReferenceNode.PropertyInspectorName);
            }
            else if (m_referenceSamplerId > -1 && m_referenceNode != null)
            {
                m_additionalContent.text =
                    string.Format(Constants.PropertyValueLabel, m_referenceNode.PropertyInspectorName);
            }
            else
            {
        ...
        public override void DrawProperties()
        {
            ...
            if (arr != null && arr.Count > 0)
            {
                ...
                m_referenceSamplerId = EditorGUILayoutPopup(Constants.AvailableReferenceStr, m_referenceSamplerId + 1,
                    arr.ToArray()) - 1;
            }
            else
            {
            ...
            bool ignoreLocalvar)
        {
            ...
            else
            {
                ...
                UIUtils.ShowMessage(UniqueId, "Please specify a texture sample on the Texel Size node",
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            EditorGUI.BeginChangeCheck();
            {
                ...
                if (arr != null && arr.Count > 0)
                {
                    arr.Insert(0, "None");
                    GUI.enabled = true && (!m_inputPorts[0].IsConnected);
                    m_referenceSamplerId =
                        m_upperLeftWidget.DrawWidget(this, m_referenceSamplerId + 1, arr.ToArray()) - 1;
                }
                else
                {
                    m_referenceSamplerId = -1;
