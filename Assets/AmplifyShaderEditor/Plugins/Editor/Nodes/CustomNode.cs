...
namespace AmplifyShaderEditor
{
    ...
    {
        [SerializeField] private List<string> m_includes;

        [SerializeField] private List<CustomNodeOutputData> m_outputData;
            ...
            if (outputId < m_outputData.Count)
            {
                ...
                for (int i = 0; i < m_outputData[outputId].inputData.Count; i++)
                {
                    if (inputs.ContainsKey(m_outputData[outputId].inputData[i].name))
                    {
                        ...
                        if (inputPort != null)
                        {
                            string inputValue = inputPort.GenerateShaderForOutput(ref dataCollector,
                                WirePortDataType.OBJECT, ignoreLocalvar);
                            value = value.Replace(m_outputData[outputId].inputData[i].name, inputValue);
                        }
                        else
                        {
                            UIUtils.ShowMessage(UniqueId,
                                m_outputData[outputId].inputData[i].name + " invalid on the inputs list",
                    ...
                    {
                        UIUtils.ShowMessage(UniqueId,
                            m_outputData[outputId].inputData[i].name + " Not found on the inputs list",
                            MessageSeverity.Error);
                        return string.Empty;
                    }
                }
