...
{
    [Serializable]
    [NodeAttributes("Template Local Var Data", "Surface Data",
    ...
    public sealed class TemplateLocalVarsNode : TemplateNodeParent
    {
        ...
        [SerializeField] private int m_currentDataIdx = -1;

        [SerializeField] private string m_dataName = string.Empty;
        ...
        void UpdateFromId()
        {
            if (m_localVarsData != null)
            {
                ...
                bool areCompatible = TemplateHelperFunctions.CheckIfCompatibles(m_outputPorts[0].DataType,
                    m_localVarsData[m_currentDataIdx].DataType);
                ...
                switch (m_localVarsData[m_currentDataIdx].DataType)
                {
                    ...
                    case WirePortDataType.FLOAT:
                        m_outputPorts[0].ChangeProperties(Constants.EmptyPortValue,
                            m_localVarsData[m_currentDataIdx].DataType, false);
                        break;
            ...
            bool ignoreLocalvar)
        {
            ...
            {
                UIUtils.ShowMessage(UniqueId,
                    string.Format("Local Var {0} can only work on ports of type {1}",
                        m_localVarsData[m_currentDataIdx].LocalVarName, m_localVarsData[m_currentDataIdx].Category));
            ...
            if (m_multiPassMode)
            {
                ...
                {
                    UIUtils.ShowMessage(UniqueId,
                        string.Format("{0} is only intended for subshader {1} and pass {2}",
                            m_dataLabels[m_currentDataIdx], SubShaderIdx, PassIdx));
