...
namespace AmplifyShaderEditor
{
    ...
    public class TemplateAdditionalParentHelper : TemplateModuleParent
    {
        ...
        [SerializeField] protected List<string> m_nativeItems = new List<string>();

        [SerializeField] protected bool m_nativeItemsFoldout = false;

        [SerializeField] protected List<string> m_additionalItems = new List<string>();

        [SerializeField] protected List<string> m_outsideItems = new List<string>();
        ...
        void DrawMainBody()
        {
            ...
            for (int i = 0; i < itemCount; i++)
            {
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    // Add new port
                    if (m_currentOwner.GUILayoutButton(string.Empty, UIUtils.PlusStyle,
                    ...
                    //Remove port
                    if (m_currentOwner.GUILayoutButton(string.Empty, UIUtils.MinusStyle,
                            GUILayout.Width(ShaderKeywordButtonLayoutWidth)))
                    {
                        markedToDelete = i;
                        m_isDirty = true;
        ...
        public virtual void AddToDataCollector(ref MasterNodeDataCollector dataCollector,
            TemplateIncludePragmaContainter nativesContainer)
        {
        }
        ...
        {
            get { return m_additionalItems; }
            set { m_additionalItems = value; }
        }

        public List<string> OutsideList
        {
            get { return m_outsideItems; }
            set { m_outsideItems = value; }
        }
    }
}