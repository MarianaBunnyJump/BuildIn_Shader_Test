...
namespace AmplifyShaderEditor
{
    ...
    public class PaletteParent : MenuParent
    {
        ...
        public PaletteParent(AmplifyShaderEditorWindow parentWindow, float x, float y, float width, float height,
            string name, MenuAnchor anchor = MenuAnchor.NONE, MenuAutoSize autoSize = MenuAutoSize.NONE) : base(
            parentWindow, x, y, width, height, name, anchor, autoSize)
        {
        ...
        public override void Draw(Rect parentPosition, Vector2 mousePosition, int mouseButtonId, bool hasKeyboadFocus)
        {
            ...
            GUILayout.BeginArea(m_transformedArea, m_content, m_style);
            {
                ...
                if (currenEvent.type == EventType.KeyDown)
                {
                    ...
                    if ((currenEvent.keyCode == KeyCode.KeypadEnter || currenEvent.keyCode == KeyCode.Return) &&
                        currenEvent.type == EventType.KeyDown)
                    {
                        int index = m_currentItems.FindIndex(x =>
                            GUI.GetNameOfFocusedControl().Equals(x.ItemUIContent.text + m_resizable));
                        if (index > -1)
                ...
                m_currentScrollPos = EditorGUILayout.BeginScrollView(
                    m_currentScrollPos /*, GUILayout.Width( 242 ), GUILayout.Height( 250 - 2 - 16 - 2 - 7 - 2) */);
                {
                    if (m_forceUpdate)
                    {
                        m_forceUpdate = false;
                        ...
                        if (usingSearchFilter)
                        {
                            for (int i = 0; i < allItems.Count; i++)
                            {
                                ...
                                {
                                    m_currentCategories.Add(allItems[i].Category,
                                        new PaletteFilterData(m_defaultCategoryVisible));
                    ...
                    if ((currenEvent.keyCode == KeyCode.DownArrow || currenEvent.keyCode == KeyCode.UpArrow) &&
                    ...
                    if (currenEvent.keyCode == KeyCode.DownArrow && currenEvent.type == EventType.KeyDown)
                    {
                        ...
                        int nextIndex = m_currentItems.FindIndex(x =>
                            GUI.GetNameOfFocusedControl().Equals(x.ItemUIContent.text + m_resizable)) + 1;
                    ...
                    {
                        ContextMenuItem item = m_currentItems.Find(x =>
                            GUI.GetNameOfFocusedControl().Equals(x.ItemUIContent.text + m_resizable));
                        if (item != null)
                        {
                                    ...
                                    //else
                                    {
                                        ...
                                        //if ( m_resizable )
                                        {
                                            if (GUI.RepeatButton(thisRect, string.Empty, EditorStyles.label))
                                            {
                                                int controlID = GUIUtility.GetControlID(FocusType.Passive);
                                                GUIUtility.hotControl = controlID;
                                                OnPaletteNodeCreateEvt(current.Value.Contents[i].NodeType,
                                                    current.Value.Contents[i].Name, current.Value.Contents[i].Function);
                                        ...
                                        EditorGUI.Toggle(thisRect, current.Value.Contents[i].ItemUIContent.text, false,
                                            EditorStyles.label);
        ...
        public void DumpAvailableNodes(bool fromCommunity, string pathname)
        {
                ...
                if (fromCommunity && current.Value.HasCommunityData || !fromCommunity)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                           ...
                           )
                        {
                            ...
                            if (current.Value.Contents[i].NodeAttributes.FromCommunity)
                                communityText = "<small class=\"cardauthor\">( originally by " +
                                                current.Value.Contents[i].NodeAttributes.Community + " )</small> ";

                            string nodeInfoBody = string.Format(nodeInfoBodyFormat, pictureFilename, pageFilename,
                                nodeFullName, nodeDescription, communityText);
            ...
            // Save file
            string nodesPathname =
                pathname + (fromCommunity ? "AvailableNodesFromCommunity.txt" : "AvailableNodes.txt");
        ...
        {
            List<ContextMenuItem> allList = forceAllItems
                ? ParentWindow.ContextMenuInstance.ItemFunctions
                : ParentWindow.ContextMenuInstance.MenuItems;
        ...
        public Dictionary<string, PaletteFilterData> BuildFullList(bool forceAllNodes = false)
        {
            ...
            if (m_searchFilter.Length > 0 || !m_isActive || m_currentCategories.Count == 0)
            {
                ...
                List<ContextMenuItem> allItems = forceAllNodes
                    ? ParentWindow.ContextMenuInstance.ItemFunctions
                    : ParentWindow.ContextMenuInstance.MenuItems;
